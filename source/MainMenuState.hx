...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		cg = new FlxSprite();
		cg.frames = Paths.getSparrowAtlas('MENU/MENU-Bumpin');
		cg.antialiasing = ClientPrefs.globalAntialiasing;
		cg.animation.addByPrefix('idle',"demo menu bumpin",24);	
		cg.animation.addByPrefix('fix',"demo menu bumpin",24,false);		
		cg.animation.addByPrefix('tape',"demo menu bumpin",24,false);	
		cg.animation.addByIndices('fix2', 'demo menu bumpin', CoolUtil.numberArray(39, 25), "", 24);
		cg.animation.addByIndices('tape2', 'demo menu bumpin', CoolUtil.numberArray(26, 8), "", 24);
		cg.animation.play('idle');
		cg.x -= 850;
		cg.y -= 500;
		FlxTween.tween(cg,{y: -100}, 1.4, {ease: FlxEase.expoInOut});
		cg.angle = -4;
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "ModPack Demo V4 version : " + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
